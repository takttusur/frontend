...
const RecipeEditorComponent = (props: RecipeTableComponentProps) => {
    ...
    const handleSaveRecipe = () => {
        ...
        const ingredientNames = newRecipe.ingredients.map((ingredient) =>
        ...
        const invalidQuantity = newRecipe.ingredients.some(
            (ingredient) => ingredient.Qty <= 0
        )
        if (invalidQuantity) {
        ...
        const dishNameExists = recipes.some(
            (recipe) =>
                recipe.name.trim().toLowerCase() ===
                newRecipe.name.trim().toLowerCase()
        )
        if (dishNameExists) {
    ...
    return (
        <Box>
            ...
            />
            <Checkbox
                isChecked={newRecipe.suggestedFor.includes(
            ...
            <RadioGroup
                ...
                onChange={(value) =>
                    setNewRecipe({ ...newRecipe, dishType: value as DishType })
                }
            >
