...
export default function WallpaperCarousel(props:IWallpaperCaruselProps) {
  ...
  useEffect(() => {
    const prevSlide = () => {
      setState((s) => ({...s,currentSlide:s.currentSlide === 0 ? slidesCount - 1 : s.currentSlide - 1 }));
    };
    
    const nextSlide = () => {
      setState((s) => ({...s,currentSlide:s.currentSlide === slidesCount - 1 ?  0 : s.currentSlide + 1}));
    };

    const automatedSlide = setInterval(() => {
      if (state.stopSliding)return;
